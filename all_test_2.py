...
def test_straggler(numIterations, fileSize, scaleSize):
    ...
    for it in tqdm(range(numIterations)):
        ...
        print("refreshing...")
        print("task start...")
        f = subprocess.Popen(read_cmd, shell=True, stdout=subprocess.PIPE)
        f.wait()
...
for p in P:
    if countP >= startP:
        for t in T:
            if countT >= startT:
                stress_process = multiprocessing.Process(
                    target=stress_generator, args=(p, t))
